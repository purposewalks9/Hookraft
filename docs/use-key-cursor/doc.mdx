...
<PropsTable
  data={[
    {
      name: "keys",
      type: "Record<string, RefObject<HTMLElement> | string>",
...
The cursor element is appended directly to `document.body` and removed on unmount — no portals or extra wrappers needed
`z-index` is `999999` so the cursor renders above modals and overlays
